...
constexpr bool is_binary(char const c) { return c == '0' || c == '1'; }

std::vector<std::byte> parse(std::string_view message) {
...
char decrypt(std::byte b) {
  b ^= INVERT_MASK;

  b = ((b & FIRST_HALF_MASK) >> 4) | ((b & SECOND_HALF_MASK) << 4);
