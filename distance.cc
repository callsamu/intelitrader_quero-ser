...
uint64_t distance(int64_t const a, int64_t const b) { return std::abs(a - b); }

void insertion_sort(std::vector<int64_t>& v) {
...
uint64_t shortest_distance(std::vector<int64_t> const& A,
  ...
  while (cursor < selected.size()) {
    if (selected.at(cursor) >= on_hold.at(on_hold_cursor)) {
      min_diff = std::min(min_diff, distance(selected.at(cursor - 1),
  ...
  min_diff = std::min(
      min_diff, distance(selected.at(cursor - 1), on_hold.at(on_hold_cursor)));

  return min_diff;
}

void print_vector(std::string_view name, std::vector<int64_t> v) {
  std::cout << name << ": ";
  for (const auto i : v) {
    std::cout << i << " ";
  }
